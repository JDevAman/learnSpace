...
public class SecondMaxEle {
    ...
    static void solve(FastReader fr, PrintWriter out) {
        ...
        Integer maxEle = nums.get(0);
        Integer secondMaxEle = Integer.MIN_VALUE;
        for(int i=1; i<nums.size(); i++){
            if(nums.get(i) > maxEle){ 
                secondMaxEle = maxEle;
                maxEle = nums.get(i);
            }
            else if(nums.get(i) > secondMaxEle){
                secondMaxEle = nums.get(i);
            }
        }
        out.println("SecondMaxEle: "+secondMaxEle);
    }
}